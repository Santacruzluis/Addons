...
class hr_special_days(models.Model):
    ...
    saturdays = fields.Integer(
        "Sabados", compute="_compute_fin_semana", store=True, readonly=True
    )
    sundays = fields.Integer(
        "Domingos", compute="_compute_fin_semana", store=True, readonly=True
    )
    saturdays_sundays_vac = fields.Integer(
        compute="_compute_cant_sab_dom_vac_nom"
    )  # nuevo
    mondays = fields.Integer(
        "Nro lunes",
        compute="_compute_days",
        help="este campo trae el numero de lunes",
        store=True,
        readonly=True,
    )
    mondays_activo = fields.Integer(
        "Nro lunes Activo",
        compute="_compute_mondays_activo",
        help="este campo trae el numero de lunes activos o laborales",
        store=True,
        readonly=True,
    )  # nuevo2
    workdays_periodo = fields.Integer(
        "Dias Laborales del Período", compute="_compute_days"
    )  # nuevo
    workdays_periodo2 = fields.Integer(
        "Dias Laborales del Período", compute="_compute_dias_periodo"
    )  # nuevo2
    saturdays_sundays_act = fields.Integer(
        compute="_compute_cant_sab_dom_act"
    )  # nuevo2

    workdays = fields.Integer(
        "Dias Laborales Activos",
        help="este campo los dias habiles del periodo",
        compute="_compute_workdays",
        store=True,
        readonly=True,
    )  # nuevo2
    holydays = fields.Integer(
        "Dias Festivos", compute="_compute_days", readonly=True
    )
    hollydays_str = fields.Integer(
        "Descansos Trabajados", compute="_compute_desfer_laborados"
    )
    hollydays_ftr = fields.Integer(
        "Feriados Trabajados", compute="_compute_desfer_laborados"
    )
    days_attended = fields.Integer(
        string="Días asistidos", compute="_compute_days_attended"
    )
    days_inasisti = fields.Integer(
        string="Dias Inasistidos", compute="_compute_days_inasisti"
    )  # odoo 14

    horas_extras_diurnas = fields.Float(
        compute="_compute_horas_extras_diurnas"
    )
    ...
    dias_utilidades = fields.Integer(
        compute="_compute_dias_utilidades", store=True
    )  # nuevo2
    dias_por_antiguedad = fields.Integer(
        compute="compute_dias_por_ano_antiguedad"
    )
    ...
    dias_vacaciones_en_nomina = fields.Integer(
        compute="_compute_cant_vac_nom"
    )  # nuevo2
    ...
    abono_check = fields.Boolean(
        default=False, string="Monto Abonos adicionales"
    )
    abono_value = fields.Float(default=0)
    ########################33 CAMPO PARA DEDUCCIONES ADICIONALES ###############################
    salary_deduction_check = fields.Boolean(
        default=False, string="Monto Deducciones"
    )
    salary_deduction_value = fields.Float(default=0)
    deduction_sc_check = fields.Boolean(
        default=False, string="Deducciones sin cobrar"
    )
    deduction_sc_value = fields.Float(default=0)
    ########################33 dias pendientes ADICIONALES ###############################
    dias_pend_check = fields.Boolean(
        default=False, string="Dias pendientes por pagar"
    )
    ...
    anticipo_vac_check = fields.Boolean(
        default=False, string="Anticipos de Vacaciones"
    )
    ...
    fecha_hoy = fields.Date(
        default=lambda *a: datetime.now().strftime("%Y-%m-%d"), readonly=True
    )  # ojo
    ...
    os_currecy_rate_gene_aux = fields.Float(
        compute="_compute_tasa_odoo", digits=(12, 4)
    )
    ...
    def _compute_dif_ingreso(self):
        for selff in self:
            delta = 0
            if (
                selff.contract_id.date_start
                and selff.contract_id.date_start >= selff.date_from
            ):
                delta = selff.days_dife(
                    selff.date_from, selff.contract_id.date_start
                )
        ...
        for selff in self:
            ...
            if selff.contract_id.date_end:
                if selff.contract_id.date_end <= selff.date_to:
                    beta = selff.days_dife(
                        selff.contract_id.date_end, selff.date_to
                    )
        ...
        for selff in self:
            if selff.company_id.tipo_dif_dias == "fech_con":
                if selff.struct_id.shedule_pay_value == 15:
                    valor = 11 - selff.valida_cant_dia_feriado(
                        selff.date_from, selff.date_to
                    )
                if selff.struct_id.shedule_pay_value == 30:
                    valor = 22 - selff.valida_cant_dia_feriado(
                        selff.date_from, selff.date_to
                    )
                if selff.struct_id.shedule_pay_value == 7:
                    valor = 5 - selff.valida_cant_dia_feriado(
                        selff.date_from, selff.date_to
                    )
    ...
    def _compute_mondays_activo(self):
        if self.struct_id.tipo_struct != "vac":
            for selff in self:
                ...
                op1 = selff.env["hr.leave"].search(
                    [
                        ("holiday_status_id.code", "=", "VAC"),
                        ("employee_id", "=", selff.employee_id.id),
                        ("state", "=", "validate"),
                        ("request_date_to", ">=", selff.date_from),
                        ("request_date_to", "<=", selff.date_to),
                    ]
                )
                op2 = selff.env["hr.leave"].search(
                    [
                        ("holiday_status_id.code", "=", "VAC"),
                        ("employee_id", "=", selff.employee_id.id),
                        ("state", "=", "validate"),
                        ("request_date_from", "<=", selff.date_to),
                        ("request_date_from", ">=", selff.date_from),
                    ]
                )
                op3 = selff.env["hr.leave"].search(
                    [
                        ("holiday_status_id.code", "=", "VAC"),
                        ("employee_id", "=", selff.employee_id.id),
                        ("state", "=", "validate"),
                ...
                if op1:
                    for det in op1:
                        ...
                        delta = selff.days_dife(
                            det.request_date_to, selff.date_to
                        )
                ...
                if op2:
                    for det in op2:
                        delta = selff.days_dife(
                            selff.date_from, det.request_date_from
                        )
                        dia_in = selff.dia(selff.date_from)
                        mes_in = selff.mes(selff.date_from)
                        ano_in = selff.ano(selff.date_from)
                ...
                if (
                    ...
                    delta = selff.days_dife(
                        selff.contract_id.date_start, selff.date_to
                    )
                    rango = delta
                ...
                if selff.contract_id.date_end:
                    if selff.contract_id.date_end <= selff.date_to:
                        ...
                        delta = selff.days_dife(
                            selff.date_from, selff.contract_id.date_end
                        )
                        dia_in = selff.dia(selff.date_from)
                        mes_in = selff.mes(selff.date_from)
                        ano_in = selff.ano(selff.date_from)
    ...
    def _compute_workdays(self):
        for selff in self:
            ...
            if selff.company_id.tipo_dif_dias == "fech_con":
                dia = selff.struct_id.shedule_pay_value
                diferencia = dia - (
                    selff.holydays + selff.saturdays + selff.sundays
                )
            if selff.company_id.tipo_dif_dias == "fech_cal":
                # diferencia=selff.days_dife(selff.date_from,selff.date_to)+1-selff.holydays-selff.saturdays-selff.sundays#-diferencia_2
                diferencia = (
                    selff.days_dife(selff.date_from, selff.date_to)
                    - (selff.saturdays + selff.sundays)
                    - selff.holydays
                )  # -1
            ...
            if op1:
                for det in op1:  # aqui regresa de vacaciones
                    diferencia = (
                        selff.days_dife(det.request_date_to, selff.date_to)
                        - selff.resta_dia_31(selff.date_to)
                        - selff.saturdays_sundays_act
                    )  # -selff.holydays-(selff.saturdays+selff.sundays-selff.saturdays_sundays_vac)
                    ban_vac = 1
            if op2:
                for det in op2:  # aqui se va de vacaciones
                    diferencia = (
                        selff.days_dife(selff.date_from, det.request_date_from)
                        - selff.saturdays_sundays_act
                    )  # -(selff.saturdays+selff.sundays-selff.saturdays_sundays_vac)
                    """if selff.company_id.tipo_dif_dias=="fech_cal":
            ...
            if (
                ...
                diferencia = (
                    diferencia_2
                    - 1
                    * selff.valida_cant_dia_feriado(
                        selff.contract_id.date_start, selff.date_to
                    )
                    - 1 * selff.resta_dia_31(selff.date_to)
                )
    ...
    def valida_cant_dia_feriado(self, fecha_inc, fecha_fin):
        cont = 0
        busca = self.env["hr.payroll.hollydays"].search(
            [
                ("date_from", ">=", fecha_inc),
                ("date_to", "<=", fecha_fin),
                ("hollydays", "=", True),
    ...
    def _compute_cant_sab_dom_act(self):
        ...
        for selff in self:
            ...
            )  # si en tod la nomina esta de vacaciones aun

            if op1:
                cant_act = 0
                for det in op1:
                    delta = (
                        selff.days_dife(det.request_date_to, selff.date_to) - 1
                    )
            ...
            if selff.struct_id.tipo_struct != "vac":
                ...
                if op1:
                    for det in op1:
                        ...
                        i = 0
                        dia = dia_in
                        diferencia = selff.days_dife(
                            selff.date_from, det.request_date_to
                        )
                        for i in range(diferencia + 1):
                            dia_aux = 0
                            dia_aux = calendar.weekday(ano_in, mes_in, dia + i)
                ...
                if op2:
                    for det in op2:
                        ...
                        dia = dia_in
                        diferencia = selff.days_dife(
                            det.request_date_from, selff.date_to
                        )
                        for i in range(diferencia + 1):
                            dia_aux = 0
                            dia_aux = calendar.weekday(ano_in, mes_in, dia + i)
    ...
    def _compute_cant_vac_nom(self):
        for selff in self:
            ...
            if op1:
                for det in op1:
                    diferencia = (
                        selff.days_dife(selff.date_from, det.request_date_to)
                        + 1
                    )
            if op2:
                for det in op2:
                    diferencia = (
                        selff.days_dife(det.request_date_from, selff.date_to)
                        + 1
                    )
            if op3:
                for det in op3:
                    diferencia = (
                        selff.days_dife(selff.date_from, selff.date_to) + 1
                    )
            if diferencia > 0:
    ...
    def _compute_tasa_odoo(self):
        ...
        lista_tasa = self.env["res.currency.rate"].search(
            [("currency_id", "=", 2), ("name", "<=", self.fecha_hoy)],
            order="name ASC",
        )
        if lista_tasa:
    ...
    def valida_pago_repetido(self):
        rastrea = self.env["hr.payslip"].search(
            [
        ...
        if rastrea:
            raise UserError(
                _(
                    "No se puede procesar esta nomina. El empleado %s  ya se le generó esta nómina en este periódo"
                )
    ...
    def _compute_dias(self):  # Nuevo
        for selff in self:
            ...
            dias_descontar_1 = dias_descontar_2 = dias_descontar_3 = (
                dias_descontar_0
            ) = 0
            ...
            verifica1 = selff.env["hr.leave"].search(
                [
                    ("employee_id", "=", selff.employee_id.id),
                    ("state", "=", "validate"),
                    ("request_date_to", ">=", selff.date_from),
            ...
            verifica2 = selff.env["hr.leave"].search(
                [
                    ("employee_id", "=", selff.employee_id.id),
                    ("state", "=", "validate"),
                    ("request_date_from", "<=", selff.date_to),
            ...
            if verifica:
                for det in verifica:
                    if det.holiday_status_id.code == "PR":
                        dias_descontar_1 = (
                            dias_descontar_1 + det.number_of_days
                        )
                    if det.holiday_status_id.code == "PNR":
                        dias_descontar_2 = (
                            dias_descontar_2 + det.number_of_days
                        )
                    ...
                    if det.holiday_status_id.code == "VAC":
                        dias_descontar_0 = (
                            dias_descontar_0 + det.number_of_days
                        )
                        ban_vac = 1
            ...
            total_dias_justifi = (
                selff.dias_permiso_remunerado
                + selff.dias_vacaciones_pedidas
                + selff.dias_reposo_medico
                + selff.dias_reposo_medico_lab
                + selff.dias_pos_natal
                + selff.dias_peternidad
            )
            selff.dias_ausencia_injus = (
                selff.days_inasisti - total_dias_justifi
                if (selff.days_inasisti - total_dias_justifi) >= 0
                else 0
            )
    ...
    def _compute_permiso(self):
        for selff in self:
            ...
            verifica = selff.env["hr.leave"].search(
                [
                    ("employee_id", "=", selff.employee_id.id),
                    ("state", "=", "validate"),
                    ("request_date_to", ">=", selff.date_from),
            ...
            if verifica:
                for det in verifica:
                    if det.holiday_status_id.code == "RML":
                        dias_descontar_4 = (
                            dias_descontar_4 + det.number_of_days
                        )
                    if det.holiday_status_id.code == "RM":
                        dias_descontar_5 = (
                            dias_descontar_5 + det.number_of_days
                        )
                    if det.holiday_status_id.code == "DPPN":
                        dias_descontar_6 = (
                            dias_descontar_6 + det.number_of_days
                        )
                    if det.holiday_status_id.code == "LPP":
                        dias_descontar_7 = (
                            dias_descontar_7 + det.number_of_days
                        )
    ...
    def _compute_dias_utilidades(self):
        for selff in self:
            dias_utilidades = 30
            indicador = self.env["hr.payroll.indicadores.economicos"].search(
                [("code", "=", "DUT")]
            )
            if indicador:
            ...
            diferencia = selff.days_dife(
                selff.fecha_aux_util, selff.contract_id.date_start
            )
    ...
    def _compute_sueldo_mes_anterior(self):
        for selff in self:
            ...
            if mes_anterior == 0:
                mes_anterior = 12
            verifica = selff.env["hr.payroll.prestaciones"].search(
                [
                    ("employee_id", "=", selff.employee_id.id),
                    ("nro_mes", "=", mes_anterior),
                ],
                order="id desc",
            )
    ...
    def _compute_dias_vacaciones(self):
        dias_difrute = 0
        for selff in self:
            verifica = selff.env["hr.payroll.dias.vacaciones"].search(
                [("service_years", "=", selff.tiempo_antiguedad)]
            )
            if verifica:
                for det in verifica:
    ...
    def compute_dias_por_ano_antiguedad(self):
        dias_antiguedad = 0
        for selff in self:
            verifica_antig = selff.env["hr.payroll.dias.vacaciones"].search(
                [("service_years", "=", selff.tiempo_antiguedad)]
            )
            if verifica_antig:
    ...
    def _compute_days(self):  # nuevo
        for selff in self:
            selff.holydays = 0
            holydays = mondays = saturdays = sundays = workdays = (
                nro_feriado
            ) = 0
            hr_payroll_hollydays = selff.env[
                "hr.payroll.hollydays"
            ]  ## busca en la tabla de feriados
            ...
            hollyday_id = hr_payroll_hollydays.search(
                [
                    ("date_from", ">=", selff.date_from),
                    ("date_to", "<=", selff.date_to),
                    ("hollydays", "=", True),
                ]
            )
            ...
            if hollyday_id:
                for det_holyday in hollyday_id:
                    nro_feriado = 1 + selff.days_dife(
                        det_holyday.date_from, det_holyday.date_to
                    )
                    selff.holydays = holydays + nro_feriado

            if selff.company_id.tipo_dif_dias == "fech_cal":
                workdays = (
                    dif_dia - saturdays - sundays - holydays
                )  #  OJO REVISAR PARA SUPER CAUCHOS
            if selff.company_id.tipo_dif_dias == "fech_con":
                workdays = (
                    selff.struct_id.shedule_pay_value
                    - saturdays
                    - sundays
                    - holydays
                )
    ...
    def _compute_days_inasisti(self):
        for selff in self:
            selff.days_inasisti = (
                selff.workdays - selff.days_attended
                if (selff.workdays - selff.days_attended) >= 0
                else 0
            )
    ...
    def _compute_horas_extras_diurnas(self):
        for selff in self:
            ...
            cantidad_horas_dia_permitida = (
                selff.employee_id.contract_id.resource_calendar_id.hours_per_day
            )
            total_horas_dias_permitidas = (
                dias_asis * cantidad_horas_dia_permitida
            )
